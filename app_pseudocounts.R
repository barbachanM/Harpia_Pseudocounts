  ...
  observeEvent(input$run,{
    ...
    if(input$selectH == 2){
      ...
      EntropyAnalysisGroup1 = reactive({ if(!is.null(EntropyDataGroup1())){
        ...
        F2 = isolate(data$F_2)
        ### Progress Message ###
        withProgress(message = 'Creating Data Structure for Group 1...',value = 0, { 
      ...
      EntropyAnalysisGroup2 = reactive({ if(!is.null(EntropyDataGroup2())){
        ...
        ### Progress Message ###
        withProgress(message = 'Creating Data Structure Group 2...', value = 0, { 
    ...
    if(input$selectH == 3){
      ...
      EntropyAnalysisGroup1 = reactive({ if(!is.null(EntropyDataGroup1())){
        ...
        F3 = isolate(data$F_3)
        ### Progress Message ###
        withProgress(message = 'Creating Data Structure Group 1...',value = 0, { 
          ...
          for(f in f1){
            ...
            for (call in alphabetH1){
              values(CountHash$H1, keys= call) = sum(stri_count_regex(fileIN, paste(call,"\t", sep="")))+(1/nH1)
            }
            for (call in alphabetH2){
              values(CountHash$H2, keys= call) = sum(stri_count_regex(fileIN, paste(call,"\t", sep="")))+(1/nH2)
              Counts2[f][call,] = values(CountHash$H2, keys= call)
            }
            for (call in alphabetH3){
              values(CountHash$H3, keys= call) = sum(stri_count_regex(fileIN, paste(call,"\t", sep="")))+(1/nH3)
      ...
      EntropyAnalysisGroup2 = reactive({ if(!is.null(EntropyDataGroup2())){
        ...
        withProgress(message = 'Creating Data Structure Group 2...', value = 0, { 
          ...
          for(f in f1){
            ...
            for (call in alphabetH2){
              values(CountHash$H2, keys= call) = sum(stri_count_regex(fileIN, paste(call,"\t", sep="")))+(1/nH)
              Counts2[f][call,] = values(CountHash$H2, keys= call)
            }
            for (call in alphabetH3){
    ...
    EntropyAnalysisGroup2 = reactive({ if(!is.null(EntropyDataGroup2())){
      ...
      ### Progress Message ###
      withProgress(message = 'Creating data structure for Group 2...', value = 0, { 
    ...
    createMLEData = reactive({
      if(!is.null(EntropyAnalysisGroup2()) & !is.null(EntropyAnalysisGroup1())){
        ...
        for (n in rownames(EntropyData)){
          ...
          mouseData = data.frame(Mouse = c(rep(n,length(colnames(EntropyGroup1LM)))),
                                 Entropy = e,
                                 Level = factor(colnames(EntropyGroup2LM)),
                                 Genotype = {
                                   if(EntropyData[n,"Genotype"] == "Group1"){Genotype = c(rep("Group1",length(colnames(EntropyGroup1LM))))}
